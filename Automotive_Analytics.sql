...
Select
100.0* SUM(CASE WHEN DATEDIFF(DAY, orderdate, shipdate) <= 5 THEN 1 ELSE 0 END) / COUNT(*) AS OnTimeDeliveryPercent
FROM supplier

-- Repeat Supplier Rate
SELECT 
100.0 * COUNT(*) * 1.0 /
(SELECT COUNT(DISTINCT SupplierName) FROM supplier) AS RepeatSupplierRate
FROM 
(SELECT SupplierName
	FROM supplier
...
--High Discount Order % (Discount > 20%)
SELECT 
  100.0 * SUM(CASE WHEN discount > 0.20 THEN 1 ELSE 0 END) / COUNT(*) AS HighDiscountOrderPercent
  FROM supplier

--Top Car Models
SELECT CarModel, SUM(CarPrice) AS Revenue
FROM supplier
GROUP BY CarModel
